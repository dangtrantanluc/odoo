...
class ProjectHomepage(http.Controller):
    ...
    def _project_module_candidates(self):
        installed_modules = set(request.env['ir.module.module'].sudo().search([
            ('name', 'in', ['bb_project_management', 'bbsw_project_management']),
            ('state', '=', 'installed'),
        ]).mapped('name'))
        return [
            module_name
            for module_name in ('bb_project_management', 'bbsw_project_management')
            if module_name in installed_modules
        ] or ['bb_project_management', 'bbsw_project_management']
    ...
    def bb_project_redirect(self, **kw):
        for module_name in self._project_module_candidates():
            action = request.env.ref(f'{module_name}.action_bb_project_dashboard', raise_if_not_found=False)
            menu = (
                request.env.ref(f'{module_name}.menu_bb_project_dashboard', raise_if_not_found=False)
                or request.env.ref(f'{module_name}.menu_bb_project_root', raise_if_not_found=False)
            )
            if action and menu:
                return request.redirect(f'/odoo#action={action.id}&menu_id={menu.id}')
            if action:
                return request.redirect(f'/odoo#action={action.id}')

        for module_name in self._project_module_candidates():
            action = request.env.ref(f'{module_name}.action_bb_project', raise_if_not_found=False)
            menu = (
                request.env.ref(f'{module_name}.menu_bb_project_all', raise_if_not_found=False)
                or request.env.ref(f'{module_name}.menu_bb_project_root', raise_if_not_found=False)
            )
            if action and menu:
                return request.redirect(f'/odoo#action={action.id}&menu_id={menu.id}')
        ...
        for module_name in self._project_module_candidates():
            menu_root = request.env.ref(f'{module_name}.menu_bb_project_root', raise_if_not_found=False)
            if menu_root:
                return request.redirect(f'/odoo#menu_id={menu_root.id}')
...
class CustomHome(Home):
    ...
    def index(self, **kw):
        if not request.session.uid:
            return request.redirect('/web/login?redirect=/project/home')
        return request.redirect('/project/home')

    @http.route('/odoo', auth='none')
