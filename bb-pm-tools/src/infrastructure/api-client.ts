...
export type CheckinSessionDto = {
  ...
  state: "IDLE" | "AWAITING_PROJECT" | "AWAITING_UPDATE" | "AWAITING_TASK_CONFIRM" | "COMPLETED";
  expiresAt: string;
...
const API_IO_LOG = (process.env.BB_PM_API_IO_LOG ?? "true").toLowerCase() !== "false";
const API_IO_LOG_MAX_CHARS = Number(process.env.BB_PM_API_IO_LOG_MAX_CHARS ?? 2000);
const API_IO_SILENT_SUCCESS = new Set(
  (process.env.BB_PM_API_IO_SILENT_SUCCESS ?? "GET /agent/automations")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean),
);

function truncateForApiLog(value: string, max = API_IO_LOG_MAX_CHARS): string {
  if (value.length <= max) return value;
  return `${value.slice(0, max)}…[truncated ${value.length - max} chars]`;
}

function summarizeForApiLog(value: unknown): unknown {
  ...
  if (Array.isArray(obj.data)) {
    return {
      ...obj,
      data: { type: "array", length: obj.data.length, sample: obj.data.slice(0, 3) },
    };
  }
  if (obj.data && typeof obj.data === "object") {
    const data = obj.data as Record<string, any>;
    const summarizedData: Record<string, any> = {};
    for (const [key, item] of Object.entries(data)) {
      summarizedData[key] = Array.isArray(item)
        ? { type: "array", length: item.length, sample: item.slice(0, 3) }
        : item;
    }
    return { ...obj, data: summarizedData };
  }
  return value;
}

function shouldLogApiEvent(event: string, fields: Record<string, unknown>): boolean {
  if (!API_IO_LOG) return false;
  const key = `${String(fields.method ?? "")} ${String(fields.path ?? "")}`;
  if (event === "bbpm.request" && API_IO_SILENT_SUCCESS.has(key)) return false;
  if (event === "bbpm.response" && fields.ok === true && API_IO_SILENT_SUCCESS.has(key)) return false;
  return true;
}

function apiLog(event: string, fields: Record<string, unknown>) {
  if (!shouldLogApiEvent(event, fields)) return;
  const payload = {
  ...
  try {
    console.log(`[bb-pm-tools/api-io] ${truncateForApiLog(JSON.stringify(payload))}`);
  } catch {
    console.log(`[bb-pm-tools/api-io] ${event}`);
  }
}
...
class BbPmClient {
  private async request<T>(
    ...
    apiLog("bbpm.request", {
      method,
      path,
      url: url.toString().replace(config.bbPmApi.baseUrl.replace(/\/$/, ""), ""),
      body: summarizeForApiLog(body),
    });
    try {
      ...
      apiLog("bbpm.response", {
        ...
        ok: true,
        durationMs: Date.now() - startedAt,
        response: summarizeForApiLog(json),
      });
      return json;
    } catch (err: any) {
      if (!String(err?.message ?? err).startsWith("bb-pm API ")) {
        apiLog("bbpm.error", {
          method,
          path,
          durationMs: Date.now() - startedAt,
  ...
  async findUser(name: string) {
    ...
    return {
      data: filtered.map((u) => ({ id: u.id, name: u.fullName, email: u.email, role: u.role })),
    };
  }
  ...
  // Reverse lookup: Gapo user id / conversation thread id → bb-pm user.
  // Returns null if no mapping exists (caller is a stranger or import didn't include them).
  async getUserByGapoCid(externalId: string, threadId?: string): Promise<{
    id: number; email: string; fullName: string; role: string; active: boolean;
  } | null> {
    try {
      const params = new URLSearchParams({ channel: "gapo", externalId });
      if (threadId) params.set("threadId", threadId);
      const res = await this.get<{
        data: {
          user: { id: number; email: string; fullName: string; role: string; active: boolean };
        };
      }>(`/agent/user-by-channel?${params.toString()}`);
  ...
  importCheckin(body: {
    ...
    return this.post<{
      data: { id: number; taskId: number | null; projectId: number; workDate: string; hours: number; description: string };
    }>("/agent/checkins/import", body);
  }


  updateCheckinWorklog(backlogId: number, body: {
    userId: number;
    workDate?: string;
    hours?: number;
    description?: string;
    ...
    return this.patch<{
      data: { id: number; taskId: number | null; projectId: number; workDate: string; hours: number; description: string };
    }>(`/agent/checkins/${backlogId}`, body);
  }

  startCheckinSession(body: {
    userId: number;
    gapoUserId: string;
    threadId: string;
    ...
    return this.post<{ data: CheckinSessionDto }>("/agent/checkin-sessions/start", body);
  }

  getCheckinProjects(userId: number) {
    return this.get<{ data: ProjectListItem[] }>("/agent/checkins/projects", { userId });
  }

  getCurrentCheckinSession(userId: number) {
    return this.get<{ data: CheckinSessionDto }>("/agent/checkin-sessions/current", { userId });
  }
  ...
  patchCheckinSession(
    ...
    body: Partial<Pick<
      CheckinSessionDto,
      "currentProjectId" | "currentTaskId" | "state" | "expiresAt" | "lastMessageId" | "pendingText" | "pendingParsed"
    >>,
  ) {
    return this.patch<{ data: CheckinSessionDto }>(`/agent/checkin-sessions/${id}`, body);
  }

  completeCheckinSession(id: number) {
    return this.post<{ data: CheckinSessionDto }>(`/agent/checkin-sessions/${id}/complete`, {});
  }

  getCheckinStatus(args: { date?: string; projectId?: number; userId?: number } = {}) {
    return this.get<{ data: any[]; meta: { total: number; date: string } }>("/agent/checkins/status", args);
  }

  getMissingCheckins(args: { date?: string; projectId?: number } = {}) {
    return this.get<{
      data: Array<{
        id: number;
        fullName: string;
    ...
    }>("/agent/checkins/missing", args);
  }

  getProjectDailyCheckinSummary(args: { projectId: number; date?: string }) {
    return this.get<{
      data: {
        projectId: number;
        date: string;
    ...
    }>("/agent/checkins/project-daily-summary", args);
  }
