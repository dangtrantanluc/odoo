...
function registerCheckinWorkflowCron(name: string, schedule: string, workflow: string) {
  if (!cron.validate(schedule)) {
    console.error(`[bb-pm-tools] invalid cron expr for ${name}: ${schedule}`);
    return;
  }
  cron.schedule(schedule, async () => {
    ...
    const result = await runWorkflow(workflow, {}, {
      source: "cron",
      correlationId: `${name}-${startedAt.getTime()}`,
    });
    console.log(`[bb-pm-tools] cron ${name} ${result.ok ? "OK" : "ERROR"}: ${result.message}`);
    if (!result.ok) void notifyAdmin(`Cron "${name}" failed`, result.error ?? result.message);
  }, { timezone: config.cron.timezone });
  console.log(`[bb-pm-tools] cron scheduled: ${name} = "${schedule}" (${config.cron.timezone})`);
}
...
export async function startScheduler() {
  ...
  if (config.cron.checkinEnabled) {
    registerCheckinWorkflowCron("noon-checkin", config.cron.noonCheckin.schedule, "noon_checkin_reminder");
    registerCheckinWorkflowCron("eod-checkin", config.cron.eodCheckin.schedule, "eod_checkin_reminder");
    registerCheckinWorkflowCron(
      "missing-checkin-followup",
      config.cron.missingCheckinFollowup.schedule,
      "missing_checkin_followup",
    );
  } else {
    console.log("[bb-pm-tools] scheduler: check-in reminder cron disabled (CRON_CHECKIN_ENABLED=false)");
  }
