...
class BbProjectMemberRate(models.Model):
    ...
    project_id = fields.Many2one('bb.project', related='member_id.project_id', store=True)
    user_id = fields.Many2one('res.users', related='member_id.user_id', store=True)

    cost_per_hour = fields.Monetary(
        string='Cost / Hour', required=True, currency_field='currency_id',
    )
